...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!player.queue || !player.queue.length || player.queue.length === 0)
      return client.sendTime(message.channel, "❌ | **Não está tocando nada agora...**");
    ...
    player.queue.clear();
    await client.sendTime(message.channel, "✅ | **Limpou a fila!**");
  },
   ...
   */
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!player)
        return client.sendTime(interaction, "❌ | **Não está jogando nada agora...**");

      if (!player.queue || !player.queue.length || player.queue.length === 0)
        return client.sendTime(interaction, "❌ | **Não está jogando nada agora...**");
      player.queue.clear();
      await client.sendTime(interaction, "✅ | **Limpou a fila!**");
    },
  },
};
